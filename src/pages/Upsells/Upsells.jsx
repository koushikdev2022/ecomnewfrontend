...
import UpsellProTypeModal from "./UpsellProTypeModal";
...
const Upsells = () => {
  ...
  // console.log("Upsell List: ", upSellList?.up_sells);
  //date Formatting
  ...
  return (
    <div className="product_details_area mt-0">
      <div className="bg-white shadow-xl lg:p-12 rounded-lg mb-16">
        <div className="lg:flex mb-12">
          <div className="w-full lg:w-9/12 mr-4">
            <div className="mb-0 w-6/12">
              <div className="mb-2 block">
                <Label htmlFor="countries" value="Select upsells" />
              </div>
              <Select onChange={handleProductSelect} value={selectedProductId}>
                <option value="">Select Upsellls</option>
                {upSellList?.up_sells?.map((up) => {
                  return (
                    <>
                      <option value={up?.id}>{up?.sells_product_name}</option>
                    </>
                  );
                })}
              </Select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredProducts?.map((upsellProd) => {
            return (
              <>
                  ...
                  <div className="w-8/12 pl-4">
                    <div className="mb-4">
                      <h3 className="text-black text-[15px] font-semibold pb-2">
                        {upsellProd?.sells_product_name} Upsell
                      </h3>
                      <p className="text-[#848484] text-sm font-normal">
                        {/* 01/05/24 */}
                        {formatDate(upsellProd?.created_at)}
                      </p>
                    </div>
                    <div className="flex">
                      <Link
                        to={`/edit-upsell/${upsellProd?.id}`}
