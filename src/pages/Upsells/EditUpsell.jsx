...
import { getSingleUpSell, updateUpSell } from "../../Reducer/UpSellSlice";

const EditUpSell = () => {
  const up_sell_id = useParams();
  // console.log("down_sell_id: ", up_sell_id?.up_sell_id);
  ...
  useEffect(() => {
    dispatch(getSingleUpSell(up_sell_id?.up_sell_id)).then((res) => {
      // console.log("getSingleDownSell", res);
      const upsells = res?.payload?.data;
      setValue("sells_product_name", upsells?.sells_product_name);
      setValue("sells_product_label", upsells?.sells_product_label);
    });
  }, []);
  const onSubmit = (data) => {
    dispatch(updateUpSell({ ...data, sell_id: up_sell_id?.up_sell_id })).then(
      (res) => {
